...
import { addManager, editManager, getManager, getManagersList, removeManager } from '@/apiHandlers/admin';
...
export default function ManagersManagement() {
  ...
  const [managers, setManagers] = useState<any[]>([]) // @todo type
  ...
  // --- Handlers ---
  const toggleExpand = (id: number, mode: 'details' | 'modify', manager?: any) => { // @todo create manager type
  ...
  const handleUpdate = async(id: number) => {
    // In a real app, you'd pull these from a ref or local state for that specific manager
    ...
    <div className="space-y-8 pb-20 font-sans">
      ...
      <div className="space-y-4">
        {managers.map((manager) => (
          <div key={manager.id} className="bg-white dark:bg-[#1e2330] rounded-[2rem] border border-slate-200 dark:border-white/10 overflow-hidden transition-all duration-500 shadow-sm">
            ...
            <div className="p-6 flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 font-bold mt-1 uppercase tracking-tighter">{manager.email}</p>
                </div>
              </div>
              ...
              {/* <div className="grid grid-cols-2 md:grid-cols-3 gap-8">
                <div>
                  ...
                  <span className="text-[11px] font-bold dark:text-slate-200">{manager.hired}</span>
                </div>
                <div>
                  <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-1">Streak</span>
                  <span className={`text-[11px] font-black ${manager.streak > 10 ? 'text-green-500' : 'text-amber-500'}`}>{manager.streak} Days</span>
                </div>
                <div className="hidden md:block">
                  <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-1">Status</span>
                  <span className={`px-2 py-0.5 rounded-md text-[8px] font-black uppercase ${manager.status === 'active' ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'}`}>{manager.status}</span>
                </div>
              </div> */}
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  {expanded.id === manager.id && expanded.mode === 'details' ? 'Hide Details' : 'See Details'}
                </button> */}
              </div>
            </div>
                  ...
                  <div className="flex flex-col  gap-8 justify-between">
                    ...
                    <div className="flex-1 max-w-2xl">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <input onChange={(e)=>setUpdatingInputs(curr => ({ ...curr, name: e.target.value }))} value={updatingInputs.name} id={`name-${manager.id}`} type="text" defaultValue={manager.name} placeholder="Name" className="bg-white dark:bg-black/40 dark:text-gray-300 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2 text-[11px] font-bold outline-none" />
                        <input onChange={(e)=>setUpdatingInputs(curr => ({ ...curr, email: e.target.value }))} value={updatingInputs.email} id={`email-${manager.id}`} type="email" defaultValue={manager.email} placeholder="Email" className="bg-white dark:bg-black/40 dark:text-gray-300 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2 text-[11px] font-bold outline-none" />
                    ...
                    <ul className="space-y-4">
                      <li className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] font-bold dark:text-slate-200">{manager.behavior.history}</p>
                        </div>
                      </li>
                      <li className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] font-bold dark:text-slate-200">{manager.behavior.trends}</p>
                        </div>
                      </li>
                      <li className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] font-bold dark:text-slate-200">{manager.behavior.patterns}</p>
                        </div>
                      </li>
                    </ul>
                  </div>
