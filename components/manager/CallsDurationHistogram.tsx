...
export function CallDurationHistogram({triggerPerAgentSearch, agentsSelected, fromDate, toDate}:{triggerPerAgentSearch:boolean, agentsSelected:number[], fromDate: string, toDate: string}) {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        console.error("Histogram fetch error:", error);
        setData([])
      } finally {
        setIsLoading(false);
      }
  ...
  return (
    <div className="bg-white dark:bg-[#1e2330] p-8 rounded-[2.5rem] border border-slate-200 dark:border-white/10 shadow-sm mt-8">
      ...
      {isLoading ? (
        <div className="h-[480px] w-full flex flex-col items-center justify-center">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mt-6 animate-pulse">
            Calculating Distribution Density...
          </p>
        </div>
      ) : (
        <div className="animate-in fade-in duration-700">
          ...
          {/* Insight Footer - Only shown when data is ready */}
